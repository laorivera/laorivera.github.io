...
export class PantsBoxObject {
   ...
   state: StateSelection = {
    ...
    Sm = new Smanager();
    Fm = new FetchManager(this.Sm, 'pants');
    ...
    set classSelection(value: string) {
      // Reset all selections when class changes
        ...
        this.Fm.fetchList_Items('/pantslist/');
 ...
 onChangeRating(event: number) {
  ...
  onChangeEnchantment_TypeUncommon(event: string){
    ...
    this.Fm.fetchEnchantment_Value("/enchantmentlistpants/");
    if(this.Sm.getselectedItem()){this.Fm.fetchEnchantment_Value("/enchantmentlistpants/")}
  ...
  //@Output() selectionpants = new EventEmitter<any>();
}
